...
int gst_pipeline(void)
{
    ...
    clock_t First_Begin, First_End, Sec_Begin, Sec_End, Third_Begin, Third_End;
    double Stamp1_duration, Stamp2_duration, Stamp3_duration;
    unsigned int frame_num = 1,cal_num = 1;
    ...
    ASSERT(ret == 0);
    First_End = clock();
    Stamp1_duration = double(First_End - First_Begin) / CLOCKS_PER_MSEC;
    cout << "\r\n\rCam_Open run time = " << Stamp1_duration << endl;
    ...
    Sec_End = clock();
    Stamp2_duration = double(Sec_End - Sec_Begin) / CLOCKS_PER_MSEC;
    cout << "\r\n\rCam_init run time = " << Stamp2_duration << endl;
	...
	Third_Begin = clock();
        ...
        Third_End = clock();
        Stamp3_duration = double(Third_End - Third_Begin) / CLOCKS_PER_MSEC;
        cout << "\r\n\rRead_Image_data run time = " << Stamp3_duration << endl;
        // 'q' for termination
        if (getch() == 'q')
        {
            endwin();
